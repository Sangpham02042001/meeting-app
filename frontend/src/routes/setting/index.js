import React, { useState } from 'react'
import { useSelector, useDispatch } from 'react-redux'
import { FormControlLabel, Switch } from '@mui/material'
import { styled } from '@mui/material/styles'
import { toggleDarkMode, toggleMute } from '../../store/reducers/setting.reducer'
import SwitchDarkMode from '../../components/SwitchDarkMode'
import './setting.css'

const AudioSwitch = styled(Switch)(({ theme }) => ({
    padding: 8,
    '& .MuiSwitch-track': {
        borderRadius: 22 / 2,
        '&:before, &:after': {
            content: '""',
            position: 'absolute',
            top: '50%',
            transform: 'translateY(-50%)',
            width: 16,
            height: 16,
        },
        '&:before': {
            backgroundImage: `url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" height="16" width="16" viewBox="0 0 24 24"><path fill="${encodeURIComponent(
                theme.palette.getContrastText(theme.palette.primary.main),
            )}" d="M21,7L9,19L3.5,13.5L4.91,12.09L9,16.17L19.59,5.59L21,7Z"/></svg>')`,
            left: 12,
        },
        '&:after': {
            backgroundImage: `url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" height="16" width="16" viewBox="0 0 24 24"><path fill="${encodeURIComponent(
                theme.palette.getContrastText(theme.palette.primary.main),
            )}" d="M19,13H5V11H19V13Z" /></svg>')`,
            right: 12,
        },
    },
    '& .MuiSwitch-thumb': {
        boxShadow: 'none',
        width: 16,
        height: 16,
        margin: 2,
    },
}));

export default function Setting(props) {
    const settingReducer = useSelector(state => state.settingReducer)
    const dispatch = useDispatch()

    return (
        <div className='setting-page'>
            <h1>Setting</h1>
            <FormControlLabel
                onChange={e => { dispatch(toggleDarkMode()) }}
                control={<SwitchDarkMode sx={{ m: 1 }} checked={settingReducer.darkMode} />}
                label={settingReducer.darkMode ? 'Dark Mode' : 'Light Mode'}
            />
            <br />
            <FormControlLabel
                onChange={e => { dispatch(toggleMute()) }}
                control={<AudioSwitch sx={{ m: 1 }} checked={settingReducer.muted} />}
                label={'Mute'}
            />
        </div >
    )
}
